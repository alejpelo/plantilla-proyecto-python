...
=== "MacOS / LinuxOS"
    ...
    Para instalar Python se recomienda usar [Homebrew](https://brew.sh/) y primero instalar [pyenv](https://github.com/pyenv/pyenv). Con esta última herramienta es posible administrar múltiples versiones de Python de forma simultánea y fácil.

Posteriormente, siga los pasos descritos a continuación:

1. Cree un nuevo repositorio a partir de esta [plantilla](https://github.com/alejpelo/plantilla-proyecto-python) (_template_) con el nombre de su nuevo proyecto.
...
3. Asegúrese que _Poetry_ está usando la versión de _Python_ adecuada para crear el ambiente virtual[^1] (i.e. 3.11):
    ...
    === "WindowsOS"
        ...
        ```sh
        poetry env use "C:/Users/<USUARIO>/AppData/Local/Programs/Python/Python<>/python.exe" # (1)!
        ```

        1. Debe remplazar `<>` con la versión de _Python_ adecuada. Por ejemplo: `../Python/Python3.11/python.exe`
    ...
    === "MacOS / LinuxOS"
        ...
        1. Debe remplazar `<>` con la versión de _Python_ adecuada. Por ejemplo: `pyenv local 3.11`

4. Instale el proyecto, ejecutando:
...
6. Consulte el archivo `src/proyecto/tutorial.py` para conocer cómo usar los archivos de configuración, el log y cómo hacer _debugging_ fácilmente.

7. Consulte el archivo `docs/tutorial.md` para conocer las capacidades instaladas para la documentación del proyecto. Para trabajar en la documentación, con el ambiente virtual activado, ejecute:
    ...
    Y para desplegar la documentación en el [GitHub Pages](https://pages.github.com/), estando en la **main**/**master** _branch_, ejecute:
...
- _Code Security_: [Bandit](https://bandit.readthedocs.io/en/latest/)
...
- **main**/**master** _branch_: solo guarda los _releases_ oficiales. Cada _release_ debe estar identificado con su respectivo _tag_ que hace alusión a la versión del código (ver Versionamiento (_tags_)).

- **dev**/**develop** _branch_: funciona como integrador de los distintos ajustes, cambios, correcciones, mejoras o adiciones (_bug fixes_ o _features_) que se están desarrollando. Este es el _branch_ del que se deben crear los _forks_ para desarrollar correcciones, mejoras o adiciones en el código.

- **bugfix/** _branches_: Cada corrección debe residir en su propio _branch_. Siempre se deben crear a partir del **dev**/**develop** _branch_.

- **feature/** _branches_: Cada nuevo cambio, mejora o adición debe residir en su propio _branch_. Siempre se deben crear a partir del **dev**/**develop** _branch_.

- **release/** _branches_: Cuando se tienen acumulados varios cambios, mejoras, correcciones y/o adiciones, se crea un _branch_ de este tipo desde el **dev**/**develop** _branch_. Esto inicia el ciclo de _release_ y a partir de este momento no se pueden añadir nuevas mejoras o cambios al código, en este _branch_, que no estén relacionadas con la corrección de errores, la documentación o tareas específicas orientadas al proceso de _release_.

- **hotfix/** _branches_: Usadas para reparar rápidamente errores en la versión de producción (**main**/**master** _branch_). Este es el único _branch_ que se puede (y debe) crear a partir del **main**/**master** _branch_. Una vez terminada la corrección se fusiona nuevamente con el **main**/**master** _branch_, con el **release** _branch_ actual y con el **dev**/**develop** _branch_. La fusión con el **main**/**master** _branch_ debe llevar su respectiva **tag** con el número de versión actualizado.
