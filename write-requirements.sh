...
# requirements
NEW_REQUIREMENTS=$(uv export --no-dev --no-hashes --frozen)

if [ ! -f requirements.txt ]; then
    echo "\033[0;31mFailed: requirements.txt didn't exist!\033[0m"
    uv export --no-dev --no-hashes --frozen --format requirements-txt > requirements.txt
    FAILED=True
fi
...
# dev requirements
NEW_REQUIREMENTS_DEV=$(uv export --no-hashes --frozen)

if [ ! -f requirements-dev.txt ]; then
    echo "\033[0;31mFailed: requirements-dev.txt didn't exist!\033[0m"
    uv export --no-hashes --frozen --format requirements-txt > requirements-dev.txt
    FAILED=True
fi
...
# tests requirements
NEW_REQUIREMENTS_TESTS=$(uv export --no-dev --extra tests --no-hashes --frozen)

if [ ! -f requirements-tests.txt ]; then
    echo "\033[0;31mFailed: requirements-tests.txt didn't exist!\033[0m"
    uv export --no-dev --extra tests --no-hashes --frozen --format requirements-txt > requirements-tests.txt
    FAILED=True
fi
...
# docs requirements
NEW_REQUIREMENTS_DOCS=$(uv export --no-dev --extra docs --no-hashes --frozen)

if [ ! -f requirements-docs.txt ]; then
    echo "\033[0;31mFailed: requirements-docs.txt didn't exist!\033[0m"
    uv export --no-dev --extra docs --no-hashes --frozen --format requirements-txt > requirements-docs.txt
    FAILED=True
fi
